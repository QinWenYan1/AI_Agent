...
**注意点**
- 💡 **理解技巧**：代码库问答能教会你两件事——**怎么 prompt Claude** 才有效，以及 Claude **擅长什么 vs 需要你多引导什么**
- 💡 **理解技巧**：Claude 不只是做文本搜索——它会**深挖一层**。例如问"这个 class 怎么用"，它不会只搜类名，而是找实例化示例、调用模式，给出的答案接近 Wiki 文档的质量
- 💡 **理解技巧**：让 Claude 查 Git 历史时，不需要详细指示每一步——它会自己看 commit 是谁引入的、关联的 Issue、当时的上下文，然后给出总结
- 🔄 **知识关联**：代码库问答的效果依赖于项目 `CLAUDE.md` 提供的上下文——详见 [02-workflows.md](./02-workflows.md)
- 📋 **术语提醒**：`at-mention(@引用)` — 使用 `@` 符号引用文件/文件夹/Issue，将指定内容拉入上下文窗口
